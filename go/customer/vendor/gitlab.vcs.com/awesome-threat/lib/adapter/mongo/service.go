...
func (con *Model) FindMany(database, collection string, query *bson.M, sorts []string, offset, size int64, results interface{}) (int64, error) {
	opts := options.Find()
...
func (con *Model) AggregationCount(database, collection string, query *bson.M, fields []string) (map[string][]ResultAggregationCount, error) {
	results := map[string][]ResultAggregationCount{}
	for _, field := range fields {
		pipeline := make([]*bson.M, 0)
		pipeline = append(pipeline, &bson.M{"$match": *query})
		pipeline = append(pipeline, &bson.M{"$group": bson.M{"_id": fmt.Sprintf("$%s", field), "count": bson.M{"$sum": 1}}})
		var documents []*struct {
			ID    interface{} `json:"_id" bson:"_id"`
			Count int64       `json:"count" bson:"count"`
		}
		if err := con.Aggregate(database, collection, pipeline, &documents); err != nil {
			return nil, err
		}
		aggs := make([]ResultAggregationCount, 0)
		for _, document := range documents {
			agg := ResultAggregationCount{
				Value: document.ID,
