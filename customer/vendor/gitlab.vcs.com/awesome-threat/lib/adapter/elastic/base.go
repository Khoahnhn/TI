...
func (ss *ScrollService) Search(index string, query Query, sorts []string, size int, scrollID, keepAlive string) (*es.SearchResult, error) {
	...
	if err != nil {
		...
		var ex *es.Error
		if errors.As(err, &ex) {
			if ex.Status == http.StatusBadRequest || ex.Status == http.StatusNotFound {
				return nil, errors.New(NotFoundError)
			}
		}
		return nil, err
...
func (con *ES) SearchCollapse(index string, query Query, sorts []string, field string, offset, size int) (*es.SearchResult, error) {
	service := con.model.Search().Index(index).Collapse(es.NewCollapseBuilder(field))
	if sorts != nil && len(sorts) > 0 {
		for _, sort := range sorts {
			if strings.HasPrefix(sort, "-") {
				service = service.Sort(strings.TrimPrefix(sort, "-"), false)
...
func (con *ES) Aggregation(index string, query Query, aggregations map[string]es.Aggregation) (*es.Aggregations, error) {
	ss := con.model.Search().Index(index).Query(query).Size(0)
